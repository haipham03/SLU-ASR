...
class Inferencer:
    def __init__(self, device, huggingface_folder, models_path, lm_path) -> None:
        self.device = device
        ...
        with open(self.models_path, 'r') as file:
            ...
            for model_dir in file:
                ...
                parts = line.split()
                model_dir = parts[0]
                self.preload_model(model_dir)
                weight = float(parts[1])
                self.models.append((self.model, weight))
                self.model = Wav2Vec2ForCTC.from_pretrained(huggingface_folder).to(self.device)
 ...
 #       return pred_transcript
    ...
    def transcribe(self, wav) -> str:
        ...
        for (model, w) in self.models:
            logits = model(input_values.to(self.device)).logits
            if ens_logits == None :
                ens_logits = w * logits
            else :
                ens_logits += w * logits
        ...
        if(pred_transcript != beam_search_output[0]):
            print(pred_transcript, beam_search_output[0])
    ...
    def run(self, test_filepath):
        ...
        if filetype == 'txt':
            ...
            f = open(test_filepath.replace(filename, 'ens_transcript_'+filename), 'w+')
...
if __name__ == '__main__':
    ...
    args.add_argument('-m', '--model_path', type=str, default = 'model_list.txt',
